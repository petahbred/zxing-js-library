...
var FinderPatternFinder = /** @class */ (function () {
    ...
    FinderPatternFinder.prototype.find = function (hints) {
        ...
        ResultPoint_1.default.orderBestMultiplePatterns(patternInfo);
        return patternInfo.map(function (item, index) { return new FinderPatternInfo_1.default(patternInfo[index]); });
    };
    /**
    ...
    FinderPatternFinder.prototype.selectBestPatterns = function () {
        ...
        var unusedCenters = new Set(possibleCenters);
        var possibleCodes = [];
        if (possibleCenters.length > 3) {
            // group finders where h1xw1, h1xw2, h2xw1
            // TODO: currently does not account for top1 being a bottomleft corner
            unusedCenters.forEach(function (top1) {
                var bottomLeft;
                // remove from set so we dont compare it to itself
                unusedCenters.delete(top1);
                unusedCenters.forEach(function (top2) {
                    // are at the same height on image (possibe top corners)
                    if (top1.getY() === top2.getY()) {
                        // get horizontal distance between the two
                        var Xdistance = top2.getX() - top1.getX();
                        var Ydistance_1 = Xdistance > 0 ? Xdistance + top1.getY() : Math.abs(Xdistance) + top2.getY();
                        unusedCenters.forEach(function (bottom) {
                            if (bottom.getX() === top1.getX() && bottom.getY() === Ydistance_1 && !bottomLeft) {
