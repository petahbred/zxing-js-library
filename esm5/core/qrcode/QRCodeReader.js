...
var QRCodeReader = /** @class */ (function () {
    ...
    QRCodeReader.prototype.decodeMultiple = function (image, hints) {
        var _this = this;
        var decoderResult;
        var points;
        if (hints !== undefined && hints !== null && undefined !== hints.get(DecodeHintType_1.default.PURE_BARCODE)) {
            ...
            var detectorResult = new Detector_1.default(image.getBlackMatrix()).detectMultiple(hints);
            decoderResult = detectorResult.map(function (code) {
                return _this.decoder.decodeBitMatrix(code.getBits(), hints);
            });
            points = detectorResult.map(function (code) {
                return code.getPoints();
            });
        }
        var results = decoderResult.map(function (code, index) {
            // If the code was mirrored: swap the bottom-left and the top-right points.
            if (code.getOther() instanceof QRCodeDecoderMetaData_1.default) {
                code.getOther().applyMirroredCorrection(points[index]);
            }
            var result = new Result_1.default(code.getText(), code.getRawBytes(), undefined, points[index], BarcodeFormat_1.default.QR_CODE, undefined);
            var byteSegments = code.getByteSegments();
            if (byteSegments !== null) {
                result.putMetadata(ResultMetadataType_1.default.BYTE_SEGMENTS, byteSegments);
            }
            var ecLevel = code.getECLevel();
            if (ecLevel !== null) {
                result.putMetadata(ResultMetadataType_1.default.ERROR_CORRECTION_LEVEL, ecLevel);
            }
            if (code.hasStructuredAppend()) {
                result.putMetadata(ResultMetadataType_1.default.STRUCTURED_APPEND_SEQUENCE, code.getStructuredAppendSequenceNumber());
                result.putMetadata(ResultMetadataType_1.default.STRUCTURED_APPEND_PARITY, code.getStructuredAppendParity());
            }
            return result;
        });
        return results;
