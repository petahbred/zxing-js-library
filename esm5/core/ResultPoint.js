...
var ResultPoint = /** @class */ (function () {
    ...
    ResultPoint.orderBestMultiplePatterns = function (patterns) {
        var _this = this;
        patterns.forEach(function (code) {
            // Find distances between pattern centers
            var zeroOneDistance = _this.distance(code[0], code[1]);
            var oneTwoDistance = _this.distance(code[1], code[2]);
            var zeroTwoDistance = _this.distance(code[0], code[2]);
            var pointA;
            var pointB;
            var pointC;
            // Assume one closest to other two is B; A and C will just be guesses at first
            ...
            // we want for A, B, C. If it's negative, then we've got it flipped around and
            // should swap A and C.
            if (_this.crossProductZ(pointA, pointB, pointC) < 0.0) {
                var temp = pointA;
                pointA = pointC;
                pointC = temp;
            }
