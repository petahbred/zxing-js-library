...
export default class QRCodeReader {
    ...
    decodeMultiple(image, hints) {
        ...
        if (hints !== undefined && hints !== null && undefined !== hints.get(DecodeHintType.PURE_BARCODE)) {
            ...
            const detectorResult = new Detector(image.getBlackMatrix()).detectMultiple(hints);
            decoderResult = detectorResult.map(code => {
                return this.decoder.decodeBitMatrix(code.getBits(), hints);
            });
            points = detectorResult.map(code => {
                return code.getPoints();
            });
        }
        const results = decoderResult.map((code, index) => {
            // If the code was mirrored: swap the bottom-left and the top-right points.
            if (code.getOther() instanceof QRCodeDecoderMetaData) {
                code.getOther().applyMirroredCorrection(points[index]);
            }
            const result = new Result(code.getText(), code.getRawBytes(), undefined, points[index], BarcodeFormat.QR_CODE, undefined);
            const byteSegments = code.getByteSegments();
            if (byteSegments !== null) {
                result.putMetadata(ResultMetadataType.BYTE_SEGMENTS, byteSegments);
            }
            const ecLevel = code.getECLevel();
            if (ecLevel !== null) {
                result.putMetadata(ResultMetadataType.ERROR_CORRECTION_LEVEL, ecLevel);
            }
            if (code.hasStructuredAppend()) {
                result.putMetadata(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE, code.getStructuredAppendSequenceNumber());
                result.putMetadata(ResultMetadataType.STRUCTURED_APPEND_PARITY, code.getStructuredAppendParity());
            }
            return result;
        });
        return results;
